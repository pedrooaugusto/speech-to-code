...
export function MicrophoneButton(props: { recording: boolean; toggleRecording: () => void; }) {
    ...
    useEffect(() => {
        ...
        navigator.mediaDevices.getUserMedia({
            ...
            processor = audioContext.createScriptProcessor(2 ** 13, 1, 1);
...
function drawWaveform(width: number, height: number, context: CanvasRenderingContext2D, buffer: AudioBuffer) {
    ...
    for (let i = 0; i < width; i++) {
        ...
        context.fillRect((i * 4), (1 + min) * (height / 2), 2, Math.max(1, (max - min) * amp));
    }
}
